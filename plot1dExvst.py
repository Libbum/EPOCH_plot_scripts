'''
Plots E_x phase map vs time.

Requires directory and will use all frames available to it.

Example run case:
	python plot1dExvst.py -d ../dptest3
...
if args.tikz:
    save_ext = '.tex'
output_file = Path.cwd().joinpath('Ex' + save_ext)
...
for frame in frames:
    ...
    Ex.append(fdata['Electric Field/Ex'].data)

Ex = np.array(Ex)
tGrid = np.array(tGrid)

lxidx = find_nearest(tGrid, xlimits[0])
hxidx = find_nearest(tGrid, xlimits[1]+2) #+1 fixes pcolor indexing
tGrid = tGrid[lxidx:hxidx]

lyidx = find_nearest(xGrid, ylimits[0])
hyidx = find_nearest(xGrid, ylimits[1]+1)
xGrid = xGrid[lyidx:hyidx]

Ex = np.transpose(Ex[lxidx:hxidx,lyidx:hyidx])
