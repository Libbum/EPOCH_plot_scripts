...
proton layers here. Also, we're implimenting this directly from postpic inputs, so we can cut off 
things dynamically.
...
plotter = postpic.plotting.plottercls(fdata, outdir='./postpic/', autosave=True, project='filtDist')

species = [MS(fdata, s) for s in fdata.listSpecies()]

for part in species:
    field = part.createField(postpic.MultiSpecies.Ekin_MeV)
    plotter.plotField(field)
